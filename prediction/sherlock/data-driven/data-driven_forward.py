...
def score_lists(lists, dtm, label_var="CLUSTER"):
...
def resample_classifier(level, features, X, Y, classifier, train, validation, seed=42, list_len=25, n_samples=100, verbose=True):
	...
	df["LABEL"] = df.index
	df.to_csv("results/forward_k{:02d}_{}.csv".format(level, features), 
	...
	for n in range(n_samples):
		...
		if i == 50:
			...
			boot_df.to_csv("results/forward_k{:02d}_{}_boot.csv".format(level, features), 
						   index=None, columns=["LABEL"] + columns)
			i = 0
	boot_df["N_COMPONENTS"] = n_components
	if features == "lists":
...
def permute_classifier(level, features, X, Y, classifier, train, validation, seed=42, list_len=25, n_shuffles=100, verbose=True):
	...
	for n in range(n_shuffles):
		...
		if i == 50:
			...
			null_df.to_csv("results/forward_k{:02d}_{}_null.csv".format(level, features), 
						   index=None, columns=["LABEL"] + columns)
			i = 0
	null_df["N_COMPONENTS"] = n_components
	if features == "lists":
...
def run(level, features, list_len=25, dtm_thres=0, optimize_clf=False,
	...
	lists = pd.read_csv("../data/lists_k{:02d}_oplen.csv".format(level), index_col=None)
	scores = score_lists(lists, dtm)

	print("-" * 50 + "\nkmeans {}, {}\n".format(level, features) + "-" * 50)
	
	validation = [pmid for pmid in validation if (pmid in scores.index) and (pmid in activations.index)]
	
	fit_file = "results/forward_k{:02d}_oplen.p".format(level)
	if optimize_clf == True or not os.path.isfile(fit_file):
		...
		cv_results = search_grid(scores.loc[train], activations.loc[train], param_grid, classifier, scoring="roc_auc")
	...
	if bootstrap:
		print("")
		boot = resample_classifier(level, features, scores, activations, top_clf, 
						   		   seed=seed, train=train, validation=validation, list_len=list_len, n_samples=n_samples)

	if permutation:
		print("")
		null = permute_classifier(level, features, scores, activations, top_clf, 
								  seed=seed, train=train, validation=validation, list_len=list_len, n_shuffles=n_shuffles)

# run(7, "lists", optimize_clf=True, bootstrap=False, permutation=False)
